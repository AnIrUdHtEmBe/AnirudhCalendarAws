...
const CellModal3: React.FC<CellModalProps> = ({
  isOpen,
  onClose,
  cellData,
}) => {
  ...
  useEffect(() => {
    ...
    const setupAlwaysOnNotificationConnections = async () => {
      ...
      console.log(
        `🔄 Setting up notification connections for roomType: ${directChatRoomType}...`
      );
            ...
            try {
              ...
              const checkInitialMessages = async () => {
                ...
                try {
                  ...
                  console.log(
                    `📊 Initial notification count for ${roomKey}: ${messageCount}`
                  );
                ...
                if (messageTimestamp) {
                  ...
                  if (
                    ...
                    setNewMessagesCount((prev) => {
                      ...
                      console.log(
                        `🔴 Notification count updated for ${roomKey}: ${newCount}`
                      );
                      return {
                        ...prev,
                        [roomKey]: newCount,
              ...
              console.log(
                `✅ Notification connection established for ${roomKey}`
              );
            } catch (error) {
              console.error(
                `Failed to create notification connection for ${roomKey}:`,
                error
              );
            }
          }
  ...
  const SmallChatBox = React.memo(
    ({
                ...
                (msg) =>
                  ...
                  Math.abs(
                    new Date(msg.timestamp || msg.createdAt || 0).getTime() -
                      new Date(
                        newMsg.timestamp || newMsg.createdAt || 0
                      ).getTime()
                  ) < 1000
              );
      ...
      console.log(
        "🔍 historyBeforeSubscribe reference:",
      ...
      useEffect(() => {
        console.log(
          "🔥 useEffect triggered - historyBeforeSubscribe:",
          !!historyBeforeSubscribe,
          "loading:",
          loading
        );
        ...
        const loadMessages = async () => {
          try {
            ...
            if (currentRoom && currentRoom.handledAt) {
              ...
              console.log(
                `📊 Showing ${newMessages.length} new chat messages out of ${allMessages.length} total`
              );
      ...
      if (roomConnection) {
        try {
          ...
          const updatedRoom = updatedRooms.find(
            (room: { roomType: string }) =>
              room.roomType === smallChatBoxData.roomType
          );
          const newHandledAt = updatedRoom
            ? updatedRoom.handledAt
            : Date.now() / 1000;
  ...
  const ReplyToAllBox = React.memo(({ onClose }: { onClose: () => void }) => {
    ...
    const sendToAllUsers = async () => {
      ...
      try {
        ...
        if (successCount > 0) {
          enqueueSnackbar(
            `Message sent successfully to ${successCount} user${
              successCount > 1 ? "s" : ""
            }`,
            { variant: "success" }
          );
        ...
        if (failedUsers.length > 0) {
          enqueueSnackbar(`Failed to send to: ${failedUsers.join(", ")}`, {
            variant: "error",
          });
        }
        ...
        <div className="bg-white rounded-lg w-96 h-64 flex flex-col shadow-xl">
          ...
          <div className="flex-1 p-4 flex flex-col justify-center">
            <div className="text-sm text-gray-600 mb-3">
              This message will be sent individually to all {modalUsers.length}{" "}
              users in their respective chat rooms.
            </div>
                    ...
                    <td
                      ...
                      onClick={async (e) => {
                        ...
                        if (e.isTrusted === false) {
                          console.log(
                            "🔄 Processing automatic second click for user:",
                            user.userId
                          );
                        } else {
                          console.log(
                            "👆 Processing first click for user:",
                            user.userId
                          );
                          ...
                          setTimeout(() => {
                            console.log(
                              "🤖 Triggering automatic second click for user:",
                              user.userId
                            );
                        ...
                        try {
                          ...
                          if (openSmallChatBox) {
                            console.log(
                              "🔒 Closing existing chat before opening new one"
                            );
                            setOpenSmallChatBox(false);
                            setSmallChatBoxData(null);
                            ...
                            return new Promise((resolve, reject) => {
                              const checkRoom = () => {
                                ...
                                if (
                                  ...
                                  directChatUserId &&
                                  !directGameChatRoomName.includes("loading-")
                                ) {
                                  console.log(
                                    `✅ Room ready after ${attempts * 100}ms:`,
                                    directGameChatRoomName
                                  );
                                  resolve(true);
                                } else if (attempts >= maxAttempts) {
                                  console.error(
                                    `❌ Room not ready after ${
                                      maxAttempts * 100
                                    }ms`
                                  );
                                  ...
                                  console.log(
                                    `⏳ Waiting for room... attempt ${attempts}/${maxAttempts}`
                                  );
                          ...
                          console.log(
                            "✅ Opening SmallChatBox with room:",
                            directGameChatRoomName
                          );
            ...
            <AblyProvider client={realtimeClient}>
              <ChatClientProvider client={chatClient}>
                <ChatRoomProvider name={smallChatBoxData.roomName}>
                  <SmallChatBox
                    key={`${smallChatBoxData.userId}-${smallChatBoxData.roomName}`}
