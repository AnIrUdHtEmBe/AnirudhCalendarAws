  ...
  const fetchUserRoomData = useCallback(async (targetUserId: string) => {
    try {
      ...
      const rooms = Array.isArray(response.data)
        ? response.data
        : response.data.rooms || [];
      const nutritionRoom = rooms.find((room: any) => room.roomType === "NUTRITION");
      ...
      <div className="bg-white rounded-lg w-96 h-96 flex flex-col shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {loading ? (
            ...
            sortedMessages.map((msg: { clientId: string | number; timestamp: string | number | Date; text: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | null | undefined; }, idx: any) => {
    ...
    for (const user of users) {
      try {
        ...
        if (nutritionRoom) {
          ...
          const messageHistory = await room.messages.history({ limit: 50 });
          const messages = messageHistory.items || [];
          const seenByTeamAtDate = new Date((nutritionRoom.handledAt || 0) * 1000);
